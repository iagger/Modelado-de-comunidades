...
var resultadoPrincipal, resultado;
document.body.onload = inicio;


function inicio() {
    resultadoPrincipal = $.getJSON("http://localhost:8080/artworks/similarity/clustersHTML", function(resp) {
    var i = 1;
    $.each(resp, function(k, v) {
        $("#comuidadesDetec").append('<option name=resultado"' + i + '">Resultado' + i + '</option>');
        i++;
    });
    resultado = resultadoPrincipal.responseJSON[0];
    cargaDatos();
    });   
};

$("#comuidadesDetec").change(function() {
    var i = 1;
    $.each(resultadoPrincipal, function(key,value) {
        console.log($("#comuidadesDetec").val());
        if ($("#comuidadesDetec").val() == "Resultado" + i) {
            eliminaElementos();
            i--;
            resultado = resultadoPrincipal.responseJSON[i];
            i++;
            actualizaPantalla();
        }
        i++;
...
function cargaIndice() {
    document.querySelector('#menuIzq').insertAdjacentHTML('beforeend','<li class="nav-item" id="infor"> <a class="nav-link active" aria-current="page" href="#info"><strong><h4>Información</strong></h4></a> </li>');
};

function cargaIndiceC() {
    
    i = 0;
    $.each(resultado.Clusters, function(ki, vi) {
        document.querySelector('#menuIzq').insertAdjacentHTML(
            'beforeend',
            '<li class="nav-item" id="cltr' + i + '" href="#cltr' + i + '"> <a class="nav-link active" aria-current="page" href="#cluster' + i + '" ><strong><h4>' + "Cluster " + i + '</h4></strong></a> </li>');
        i++;
    });
};
...
function cargaCluster() {
    ...
    $.each(resultado.Clusters, function(ki, vi) {
        ...
        '<strong><h2 class="subTitulo"><u>Artistas valorados positivamente</u></h2></strong>' +
        ...
        '<strong><h2 class="subTitulo"><u>Artistas valorados negativamente</u></h2></strong>' +
...
function cargaCarrusel(i){
    ...
    var seccion = resultado.Clusters[cluster].Artistic["Most valued"];
    var j = 0;
    ...
    seccion = resultado.Clusters[cluster].Artistic["Least valued"];
    j = 0;
    $.each(seccion, function(ki, vi) {
        if (j== 0) {
...
function miAux2(dataD) {
    var arrOrd = new Map();
    $.each(dataD, function(ki, vi) {
        arrOrd.set(ki, math.round(vi*100));
    });
    arrOrd = new Map([...arrOrd.entries()].sort((a, b) => b[1] - a[1]));
    var arr = [];
    var i = 0;
    simbolos = "0123456789ABCDEF";
	miColor = "#";
    for (var [clave, valor] of arrOrd.entries()) {
        for(var j = 0; j < 6; j++){
            miColor = miColor + simbolos[Math.floor(Math.random() * 16)];
        }
        arr[i]={value: valor, label: clave, color:miColor};
        i++;
        miColor = "#";
    };
    return arr;
...
function eliminaElementos() {
    ...
    //document.getElementById("infor").parentNode.removeChild(document.getElementById("infor"));
    i = 0;
    $.each(resultado.Clusters, function(ki, vi) {
        document.querySelector("#cluster"+i).parentNode.removeChild(document.querySelector("#cluster"+i));
        document.getElementById("cltr"+i).parentNode.removeChild(document.getElementById("cltr"+i));
        i++;
    });
;}
