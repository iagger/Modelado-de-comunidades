...
class CachedSimilarity():
    ...
    def __searchEntity(self, Entity):
        if Entity not in self._cacheFile_:
            sims = dict()
            for other in iter(self._cacheFile_):
                sims |= { other : self.computeSimilarity(Entity, other) }
            self._cacheFile_[Entity] = sims
        return self._cacheFile_[Entity]
    
    def getSimilarity(self, A, B):
        dict_a = self.__searchEntity(A)
        dict_b = self.__searchEntity(B)
        if B in dict_a.keys():
            return dict_a[B]
        return dict_b[A]
